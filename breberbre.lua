		...
		function library:keybind(options)
			...
			-- init 
				...
				function cfg.set(input)
					...
					if cfg.name then 
						-- Check if parent toggle is enabled (e.g., CSyncKey -> CSyncEnabled)
						local parentToggleFlag = cfg.flag:gsub("Key$", "Enabled"):gsub("Keybind$", "Enabled")
						local parentToggleActive = true -- Default to true if no parent toggle exists
						
						-- If a parent toggle flag exists, check if it's enabled (toggles store boolean directly in flags)
						if parentToggleFlag ~= cfg.flag and flags[parentToggleFlag] ~= nil then
							parentToggleActive = flags[parentToggleFlag] == true
						end
						
						-- Only show keybind if both the keybind is active AND parent toggle is enabled
						KEYBIND_ELEMENT.Visible = cfg.active and parentToggleActive

						library:tween(KEYBIND_ELEMENT, {
							TextTransparency = (cfg.active and parentToggleActive) and 0 or 1, 
						}) 

						library:tween(KEYBIND_ELEMENT:FindFirstChildOfClass("UIStroke"), {
							Transparency = (cfg.active and parentToggleActive) and 0 or 1, 
						}) 
				...
				element_outline.MouseButton1Down:Connect(function()
					...
					-- Disconnect any existing binding first
					if cfg.binding then 
						pcall(function() cfg.binding:Disconnect() end)
						cfg.binding = nil
					end 

					-- Create new binding that auto-disconnects after first key
					local connection
					connection = library:connection(uis.InputBegan, function(input, game_event)  
						local selected_key = input.UserInputType == Enum.UserInputType.Keyboard and input.KeyCode or input.UserInputType

						-- Disconnect first to prevent re-entry
						if connection then
				...
				library:connection(uis.InputBegan, function(input, game_event) 
					...
					if not game_event then 
						if selected_key == cfg.key then 
							-- Check if parent toggle is enabled before allowing keybind activation
							local parentToggleFlag = cfg.flag:gsub("Key$", "Enabled"):gsub("Keybind$", "Enabled")
							local parentToggleActive = true -- Default to true if no parent toggle exists
							...
							-- Only activate keybind if parent toggle is enabled
							if parentToggleActive then
